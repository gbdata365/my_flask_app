    ...
    try:
        df = pd.read_sql("""
            SELECT
                ...
                numerator, denominator, multiplier, decimal_places
            FROM code_indicator
            WHERE category = 2
              AND is_active = TRUE
            ORDER BY sort_order
        ...
        return [
            ...
            {'column_name': 'pop_per_house', 'display_name': '가구당인구수',
             'description': '총인구 / 전체가구', 'numerator': 'total_pop'},
            {'column_name': 'elderly_single_ratio', 'display_name': '고령1인가구비율',
             'description': '65세이상 1인가구 / 전체 1인가구 × 100', 'numerator': 'elderly_single'}
        ]
...
def get_column_name_labels():
    ...
    try:
        ind_df = pd.read_sql("""
            SELECT column_name, display_name, numerator
            FROM code_indicator
            WHERE is_active = TRUE
        """, engine)
...
def get_household_indicator_table(base_ym_list, indicator, aggregate_type='sido', sido=None, include_sub_sigungu=False):
    ...
    cache_sigungu_indicators에 이미 계산된 지표 컬럼을 사용 (single_ratio, elderly_single_ratio 등)
    """
    engine = get_db_engine()
    col_name = indicator['column_name']  # 이미 계산된 지표 컬럼명
        ...
        if aggregate_type == 'region':
            ...
            df = pd.read_sql(f"""
                SELECT
                    ...
                    ROUND(
                        SUM(c.{col_name} * c.{denominator}) / NULLIF(SUM(c.{denominator}), 0),
                        2
                    ) as indicator_value
                FROM cache_sigungu_indicators c
                JOIN (
            ...
            if include_sub_sigungu:
                ...
                df = pd.read_sql(f"""
                    SELECT
                        ...
                        COALESCE(c.{col_name}, 0) as indicator_value
                    FROM cache_sigungu_indicators c
                    WHERE TO_CHAR(c.base_ym, 'YYYYMM') = '{ym}'
                      AND {sido_cond}
                    ORDER BY c.sigungu_code
    ...
    # 컬럼명 한글 라벨 조회
    column_labels = get_column_name_labels()
...
def create_indicator_table_html(data, ym_list):
    ...
    for i, ym in enumerate(ym_list):
        ...
        html.append(f'<th class="metric-header" style="font-size:0.85em;">{numerator_label}</th>')
        html.append(f'<th class="metric-header" style="font-size:0.85em;">{denominator_label}</th>')
        if i < len(ym_list) - 1:
    ...
    for row in rows:
        ...
        for i, ym in enumerate(ym_list):
            ...
            html.append(f'<td style="color:#666; font-size:0.9em;">{numer:,}</td>')
            html.append(f'<td style="color:#666; font-size:0.9em;">{denom:,}</td>')
...
def generate_dashboard_html(request_args):
    ...
    if not base_ym_list:
        ...
        return template.render(
            ...
            current_url='/01_population/edu_dash9',
            # 데이터 없음
            household_chart_img=None,
            single_age_chart_img=None,
            single_age_regional_chart_img=None,
            household_table_html='<p class="text-muted text-center py-5">조회 버튼을 클릭하여 데이터를 불러오세요.</p>',
            single_age_table_html='<p class="text-muted text-center py-5">조회 버튼을 클릭하여 데이터를 불러오세요.</p>',
            indicator_tabs=[{
                'id': ind['column_name'],
