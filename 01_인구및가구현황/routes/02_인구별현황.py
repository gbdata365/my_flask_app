...
from module.charts import BarChart, LineChart, ChartStyle
...
FILE_STEM = Path(__file__).stem  # '02_인구별현황'
...
def get_youth_definitions():
    """
    code_age_group에서 청년 정의 조회 (young_adult, young_adult_gb 등)

    Returns:
        dict: {
            'young_adult': {'code': 'young_adult', 'code_name': '청년(기본법)', 'age_start': 19, 'age_end': 34, 'column_name': 'young_adult_pop'},
            'young_adult_gb': {'code': 'young_adult_gb', 'code_name': '청년(경북)', 'age_start': 19, 'age_end': 39, 'column_name': 'young_adult_gb_pop'},
        }
    """
    engine = get_db_engine()
    df = pd.read_sql("""
        SELECT code, code_name, column_name, age_start, age_end
        FROM code_age_group
        WHERE category = 3
          AND is_active = TRUE
          AND code LIKE 'young_adult%'
        ORDER BY sort_order
    """, engine)
...
def get_elderly_definition():
    """
    노령인구 정의 조회 (code_age_group에서 동적으로)

    Returns:
        dict: {'column': 'elderly_pop', 'age_start': 65, 'code_name': '고령인구'}
    """
    engine = get_db_engine()
    df = pd.read_sql("""
        SELECT code, code_name, column_name, age_start, age_end
...
def generate_initial_summary_table():
    """
    초기 화면에 표시할 시도별 요약 테이블 생성
    조회 버튼을 누르기 전에도 주요 지표 개요 표시
    """
    try:
        query = CacheDataQuery('cache_sigungu_indicators')

        # 최신 년월 데이터로 시도별 요약 조회
        summary_columns = ['total_pop', 'household_cnt', 'single_cnt']
        ...
        html.append('<th class="text-center">총인구</th>')
        ...
        for _, row in df.iterrows():
            ...
            html.append(f'<td class="text-end num">{int(row.get("total_pop", 0)):,}</td>')
...
def get_age_population_by_region(base_ym_list, age_category=1, aggregate_type='sido', region=None, sido=None, priority_region='경상북도'):
    """
    지역별 연령 인구 데이터 조회 (subplot 차트용)
    - 쿼리 통합: 모든 년월을 한 번의 쿼리로 조회

    Args:
        base_ym_list: 기준년월 리스트
        age_category: 연령 카테고리 (1=5세별, 2=10세별, 3=정책연령)
        aggregate_type: 'region' (권역별), 'sido' (시도별), 'sigungu' (시군구별)
        region: 특정 권역 선택 시
        sido: 특정 시도 선택 시 (시군구별일 때 사용)
    ...
    elif aggregate_type == 'sigungu':
        # 시군구별 (특정 시도의 시군구들)
        group_col = "c.sigungu_nm"
        group_name = "sigungu_nm"
        join_clause = ""
        # 시군구 레벨만 (마지막 자리 0)
        where_clause = "AND c.sigungu_code LIKE '____0'"
        if sido:
            where_clause += f" AND c.sido_nm = '{sido}'"
        order_clause = "ORDER BY MIN(c.sigungu_code)"
    else:
        # 시도별 (캐시 테이블에 이미 정규화된 시도명)
        group_col = "c.sido_nm"
        group_name = "sido_nm"
        join_clause = """
            JOIN (
    ...
    # 쿼리 통합: 모든 년월을 한 번에 조회
    ym_list_str = "', '".join(base_ym_list)
    df = pd.read_sql(f"""
        SELECT
            TO_CHAR(c.base_ym, 'YYYYMM') as base_ym,
            {group_col} as {group_name},
            SUM(total_pop) as total_pop,
            {cols_sum}
        FROM cache_sigungu_indicators c
        {join_clause}
        WHERE TO_CHAR(c.base_ym, 'YYYYMM') IN ('{ym_list_str}')
        {where_clause}
        GROUP BY TO_CHAR(c.base_ym, 'YYYYMM'), {group_col}
    """, engine)
...
def get_age_population_table(base_ym_list, age_category=1):
    ...
    # 쿼리 통합: 모든 년월을 한 번에 조회
    ym_list_str = "', '".join(base_ym_list)
    combined_df = pd.read_sql(f"""
        SELECT
            TO_CHAR(base_ym, 'YYYYMM') as base_ym,
            SUM(total_pop) as total_pop,
            {cols_sum}
        FROM cache_sigungu_indicators
        WHERE TO_CHAR(base_ym, 'YYYYMM') IN ('{ym_list_str}')
        GROUP BY TO_CHAR(base_ym, 'YYYYMM')
    """, engine)
...
def get_region_population_table(base_ym_list):
    ...
    combined_df = pd.read_sql(f"""
        ...
        WHERE TO_CHAR(c.base_ym, 'YYYYMM') IN ('{ym_list_str}')
        GROUP BY TO_CHAR(c.base_ym, 'YYYYMM'), d.region_nm, d.region_code
...
def get_sido_population_table(base_ym_list, region=None):
    ...
    시도별 인구 현황 테이블
    - 쿼리 통합: 모든 년월을 한 번의 쿼리로 조회
    - cache 테이블의 정규화된 sido_nm, sido_alias 사용
    """
    engine = get_db_engine()
    ...
    # 쿼리 통합: 모든 년월을 한 번에 조회 (정규화된 sido_nm, sido_alias 사용)
    ym_list_str = "', '".join(base_ym_list)
    combined_df = pd.read_sql(f"""
        SELECT
            TO_CHAR(base_ym, 'YYYYMM') as base_ym,
            sido_nm,
            sido_alias,
            MIN(sido_cd) as sido_code,
            SUM(total_pop) as total_pop,
            SUM(COALESCE(household_cnt, 0)) as household_cnt
        FROM cache_sigungu_indicators
        WHERE TO_CHAR(base_ym, 'YYYYMM') IN ('{ym_list_str}')
        {where_clause}
        GROUP BY TO_CHAR(base_ym, 'YYYYMM'), sido_nm, sido_alias
        ORDER BY MIN(sido_cd)
    """, engine)
    ...
    # sido_nm → sido_alias 매핑 생성
    alias_map = combined_df.groupby('sido_nm')['sido_alias'].first().to_dict()
    ...
    # 각 시도 행 (시도코드 순) - 시도 별칭 사용
    for sido in pivot_pop.index:
        # sido_alias 사용 (2글자 축약명)
        row = {'name': alias_map.get(sido, sido)}
        for i, ym in enumerate(base_ym_list):
            if ym in pivot_pop.columns:
...
def get_youth_population_table(base_ym_list, youth_code='young_adult_gb', region=None, sido=None):
    """
    청년인구 테이블 (code_age_group 기반)

    Args:
        base_ym_list: 기준년월 리스트
        youth_code: 청년 정의 코드 (code_age_group.code)
            - 'young_adult': 청년(기본법) 19~34세
            - 'young_adult_gb': 청년(경북) 19~39세
        region: 권역 필터
        sido: 시도 필터
    """
    engine = get_db_engine()

    # 청년 정의 조회 (code_age_group에서)
    youth_defs = get_youth_definitions()
    if youth_code not in youth_defs:
        # 기본값: young_adult_gb (경북)
        youth_code = 'young_adult_gb'

    youth_def = youth_defs.get(youth_code)
    if not youth_def:
        return {'title': '청년인구', 'headers': [], 'data': []}

    age_start = youth_def['age_start']
    age_end = youth_def['age_end']
    title = f"{youth_def['code_name']} ({age_start}~{age_end}세)"
    ...
    age_cols_df = pd.read_sql(f"""
        ...
        WHERE category = 1
          AND is_active = TRUE
          AND age_start >= {max(0, age_start - 5)} AND age_end <= {age_end + 5}
        ORDER BY sort_order
    """, engine)
    ...
    for _, row in age_cols_df.iterrows():
        col = row['column_name']
        col_start = int(row['age_start'])
        col_end = int(row['age_end'])
        col_range = col_end - col_start + 1

        # 구간이 청년 범위와 겹치는 부분 계산
        overlap_start = max(col_start, age_start)
        overlap_end = min(col_end, age_end)

        if overlap_start <= overlap_end:
            overlap_range = overlap_end - overlap_start + 1
            if overlap_range == col_range:
                # 전체 포함
                youth_cols.append(f"COALESCE({col}, 0)")
            else:
                # 부분 포함 (비율 적용)
                ratio = overlap_range / col_range
    ...
    # 쿼리 통합: 모든 년월을 한 번에 조회 (캐시 테이블에 이미 정규화된 시도명)
    ym_list_str = "', '".join(base_ym_list)
    combined_df = pd.read_sql(f"""
        SELECT
            TO_CHAR(base_ym, 'YYYYMM') as base_ym,
        ...
        WHERE TO_CHAR(base_ym, 'YYYYMM') IN ('{ym_list_str}')
        {where_clause}
        GROUP BY TO_CHAR(base_ym, 'YYYYMM'), sido_nm
...
def get_sigungu_population_table(base_ym_list, sido, include_sub_sigungu=False):
    ...
    if include_sub_sigungu:
        ...
        combined_df = pd.read_sql(f"""
            ...
            GROUP BY TO_CHAR(base_ym, 'YYYYMM'), sigungu_code, sigungu_nm
        ...
        combined_df = pd.read_sql(f"""
            ...
            WHERE TO_CHAR(base_ym, 'YYYYMM') IN ('{ym_list_str}')
              AND {sido_condition}
              AND LENGTH(sigungu_code) = 5
            GROUP BY TO_CHAR(base_ym, 'YYYYMM'), LEFT(sigungu_code, 4)
    ...
    if aggregate_type == 'region':
        ...
        if include_sub_sigungu:
            ...
            combined_df = pd.read_sql(f"""
                ...
                WHERE TO_CHAR(c.base_ym, 'YYYYMM') IN ('{ym_list_str}')
                  AND {sido_cond}
                GROUP BY TO_CHAR(c.base_ym, 'YYYYMM'), c.sigungu_code, c.sigungu_nm
                ORDER BY c.sigungu_code
            """, engine)
            group_label = '시군구'
        else:
            # 하위 시군구 미포함 - 4자리 그룹화 (포항시 남구/북구 → 포항시로 집계)
            combined_df = pd.read_sql(f"""
                SELECT
                    TO_CHAR(c.base_ym, 'YYYYMM') as base_ym,
                ...
                WHERE TO_CHAR(c.base_ym, 'YYYYMM') IN ('{ym_list_str}')
                  AND {sido_cond}
                GROUP BY TO_CHAR(c.base_ym, 'YYYYMM'), LEFT(c.sigungu_code, 4)
        ...
        combined_df = pd.read_sql(f"""
            ...
            WHERE TO_CHAR(c.base_ym, 'YYYYMM') IN ('{ym_list_str}')
            {region_cond}
            GROUP BY TO_CHAR(c.base_ym, 'YYYYMM'), c.sido_alias
...
def create_indicator_chart(data, ym_list, indicator_name):
    ...
    try:
        ...
        is_horizontal = num_items >= 10

        chart = BarChart(
            title='',
            xlabel=indicator_name if is_horizontal else '지역',
            ylabel='지역' if is_horizontal else indicator_name,
        ...
        for i, ym in enumerate(ym_list):
            ...
            chart.add_series(f'{ym[:4]}.{ym[4:]}', labels, values)
...
def create_indicator_table_html(data, ym_list):
    ...
    for i, ym in enumerate(ym_list):
        ...
        html.append(f'<th class="metric-header sortable" data-col="numerator_{ym}" >{numerator_name} <span class="sort-icon"></span></th>')
        html.append(f'<th class="metric-header sortable" data-col="denom_{ym}" >{denominator_name} <span class="sort-icon"></span></th>')
    ...
    for row_idx, row in enumerate(rows):
        ...
        for i, ym in enumerate(ym_list):
            ...
            divider_class = ' divider' if i > 0 else ''
            html.append(f'<td class="num{divider_class}" data-value="{val}">{val:.2f}</td>')
            html.append(f'<td class="num" data-value="{numerator_val}" >{numerator_val:,}</td>')
            html.append(f'<td class="num" data-value="{denom_val}" >{denom_val:,}</td>')

            if change is not None:
                css_class = 'num positive' if change > 0 else 'num negative' if change < 0 else 'num'
                html.append(f'<td class="{css_class}" data-value="{change}">{change:+.2f}</td>')
            else:
                html.append('<td class="num" data-value="">-</td>')
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* 숫자는 Arial Narrow 폰트 (폭이 좁아 공간 절약) */
        td.num { font-family: 'Arial Narrow', 'DejaVu Sans Condensed', Arial, sans-serif; font-size: 12px; }
        td:first-child { text-align: center; font-weight: 500; background: #f8f9fa; white-space: nowrap; }
                ...
                for row in table_data['data']:
                    ...
                    for h in headers:
                        ...
                        if isinstance(val, float):
                            css_class = 'num'  # 숫자는 Arial Narrow
...
def create_age_chart(data, ym_list):
    ...
    try:
        ...
        is_horizontal = num_items >= 12

        chart = BarChart(
            title='',
            xlabel=f'인구 ({unit_label})' if is_horizontal else '연령대',
            ylabel='연령대' if is_horizontal else f'인구 ({unit_label})',
            horizontal=is_horizontal,
            show_labels=True,
            label_format='{:,.0f}'
        )
...
def create_region_chart(data, ym_list, chart_type='sido'):
    """지역별 인구 차트 - 연도별 막대 그래프 (module.charts 사용)"""
    if not data or not data.get('data') or len(data['data']) < 2:
        return None
    ...
    try:
        ...
        chart = BarChart(
            title='',
            xlabel=f'인구 ({unit_label})' if is_horizontal else '지역',
            ylabel='지역' if is_horizontal else f'인구 ({unit_label})',
            horizontal=is_horizontal,
            show_labels=True,
            label_format='{:,.0f}'
        )
        ...
        for ym in ym_list:
            pop_key = f'pop_{ym}'
            values = [r.get(pop_key, 0) / unit_val for r in rows]
            chart.add_series(f'{ym[:4]} {ym[4:]}', labels, values)
...
def create_age_by_region_table_html(regional_data, ym_list, max_regions=None, start_index=0):
    ...
    for idx, region_name in enumerate(regions):
        ...
        for i, ym in enumerate(ym_list):
            ...
            month = ym[4:]
            divider_class = ' divider' if i > 0 else ''
            # 마지막 년월은 증감율 없음 (비교 대상 없음)
            colspan = 1 if i == len(ym_list) - 1 else 2
            html.append(f'<th class="year-header{divider_class}" colspan="{colspan}">{year}년 {month}월</th>')
        html.append('</tr>')
        ...
        for i, ym in enumerate(ym_list):
            ...
            # 마지막 년월은 증감율 없음
            if i < len(ym_list) - 1:
                html.append('<th class="metric-header">증감율</th>')
        html.append('</tr>')
        html.append('</thead>')
        ...
        if total_pop:
            ...
            for i, ym in enumerate(ym_list):
                ...
                divider_class = ' divider' if i > 0 else ''
                html.append(f'<td class="num{divider_class}">{pop:,}</td>')
                # 증감율 계산 (다음 년월 대비)
                if i < len(ym_list) - 1:
                    next_ym = ym_list[i + 1]
                    next_pop = total_pop.get(next_ym, 0)
                    if next_pop > 0:
                        change_rate = (pop - next_pop) / next_pop * 100
                        color = '#dc3545' if change_rate < 0 else '#28a745'
                        html.append(f'<td class="num" style="color: {color};">{change_rate:+.2f}%</td>')
                    else:
        ...
        for row in age_rows:
            ...
            for i, ym in enumerate(ym_list):
                ...
                if i < len(ym_list) - 1:
                    next_ym = ym_list[i + 1]
                    next_pop = row.get(f'pop_{next_ym}', 0)
                    if next_pop > 0:
                        change_rate = (pop - next_pop) / next_pop * 100
                        color = '#dc3545' if change_rate < 0 else '#28a745'
                        html.append(f'<td class="num" style="color: {color};">{change_rate:+.2f}%</td>')
    ...
    for row_idx, row in enumerate(rows):
        ...
        for i, ym in enumerate(ym_list):
            for j, (metric_key, metric_name) in enumerate(metrics):
                ...
                if key in row:
                    val = row[key]
                    divider = ' divider' if i > 0 and j == 0 else ''

                    if isinstance(val, (int, float)):
                        if 'rate' in key:
                            sign_class = ' positive' if val > 0 else ' negative' if val < 0 else ''
                            val_str = f'{val:.2f}%'
                            html.append(f'<td class="num{sign_class}{divider}" data-value="{val}">{val_str}</td>')
                        else:
                            val_str = f'{val:,}'
                            html.append(f'<td class="num{divider}" data-value="{val}">{val_str}</td>')
                    else:
                        html.append(f'<td class="{divider.strip()}" data-value="{val}">{val}</td>' if divider else f'<td data-value="{val}">{val}</td>')
...
def generate_dashboard_html(request_args):
    ...
    # 초기 접속 시 (base_ym_list가 없으면) 요약 데이터만 표시 - 로딩 속도 최적화
    if not base_ym_list:
        # 기본 년월 설정 - 12월 데이터 중 최신 2개 (최신년월, 전년도년월)
        ...
        return template.render(
            ...
            age_table_html='<p class="text-muted text-center py-5">연령별 데이터는 조회 버튼을 클릭하세요.</p>',
            region_table_html=initial_summary_html,  # 시도별 요약 테이블 표시
    ...
    age_by_region_data = get_age_population_by_region(base_ym_list, age_category, aggregate_type, region, sido)
